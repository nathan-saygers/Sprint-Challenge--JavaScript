...
Populate the displayNames array with only the animal_name and scientific_name of each animal. displayNames 
will be an array of strings, and each string should follow this pattern: "Name: Jackal, asiatic, Scientific: Canis aureus."

*/
const displayNames = [];

zooAnimals.forEach(function(exhibit){
  return displayNames.push(`Name: ${exhibit.animal_name} Scientific: ${exhibit.scientific_name}`)
});
...
const lowCaseAnimalNames = zooAnimals.map(function(exhibit){
  return exhibit.animal_name.toLowerCase();
});
...
*/
const lowPopulationAnimals = zooAnimals.filter(function(exhibit){
  return exhibit.population < 5;
});
...
all the zoos using the .reduce() method. Remember the reduce method takes two arguments: a callback 
(which itself takes two args), and an initial value for the count.

*/
const populationTotal = zooAnimals.reduce(function(accumulator, exhibit){
  return accumulator + exhibit.population;
}, 0);
